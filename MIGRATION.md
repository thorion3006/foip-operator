...
The release may retain the existing provider route during migration, but a
handoff must be treated as a controlled operation because local `/32`
preparation is re-established before a provider mutation.
...
4. Recreate them with `spec.ip`, `spec.secretName`, and the new safety policy.
5. Reapply node annotations and verify `Ready`, `ProviderConverged`, and
...
owner, node annotations, credentials, or local interface preparation cannot be
verified. Do not delete the old resources and the provider route at the same
time without an operator-approved rollback plan.
